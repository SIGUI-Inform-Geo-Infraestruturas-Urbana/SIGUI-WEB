...
export class InfrastructureService extends DataServices{
  ...
  public convertFeature(features :Feature<Geometry>[]):Infrastructure[]{
    let streetList:Infrastructure[] = [];
    console.log('aaaaaa')
    for (let index = 0; index < features.length; index++) {

      let street = new Infrastructure().deserialize(features[index]);
